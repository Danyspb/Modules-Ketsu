...
var donnees = [ ];
var rec = document.querySelectorAll( '.result-item ' );
for ( un of rec ) {
    var link = un.querySelector('.image a').href;
    var image = un.querySelector('.image img').src;
    var title = un.querySelector( '.thumbnail.animation-2 > a > img' ).alt.trim( );
    var synap = un.querySelector('.contenido p').textContent.trim();
    link = new ModuleRequest( link, 'get', emptyKeyValue, null );
    image = new ModuleRequest( image, 'get', emptyKeyValue, null );
    var finalData = new Data( image, title, synap, '', '', '', '', false, link );;
