 ...
 var contact = new Data(rien, 'En cas de problème du module veuillez le signaler au niveau du serveur Discord de Ketsu vous pouvez y accéder en cliquant sur ce texte Allez bonne lecture 😉', '', '', '', '', '', false, lien, false);
 var sortie = [];
 ...
 for (r of reg) {
     ...
     link = new ModuleRequest(link, 'get', emptyKeyValue, null);
     var image = r.querySelector('.slider__thumb_item > a > img').dataset.src.replace('-125x180', '');
     image = new ModuleRequest(image, 'get', emptyKeyValue, null);
 ...
 for (c of check) {
     ...
     link = new ModuleRequest(link, 'get', emptyKeyValue, null);
     var image = c.querySelector('.item-thumb.c-image-hover > a > img').dataset.src.replace('-193x278', '');
     image = new ModuleRequest(image, 'get', emptyKeyValue, null);
 ...
 var sortie2 = [];
 var recup = document.querySelectorAll('.main-col.col-md-8.col-sm-8 .main-col-inner.c-page #loop-content > div .col-12.col-md-6');
 for (re of recup) {
     var title = re.querySelector('.row.row-eq-height .page-item-detail.manga > div > a').title;
     var link = re.querySelector('.row.row-eq-height .page-item-detail.manga > div > a').href;
     link = new ModuleRequest(link, 'get', emptyKeyValue, null);
     var image = re.querySelector('.row.row-eq-height .page-item-detail.manga > div > a > img').dataset.src.replace('-110x150', '');
     image = new ModuleRequest(image, 'get', emptyKeyValue, null);
     var haha = new Data(image, title, '', '', '', '', '', false, link);
     sortie2.push(haha);
 }
 var sortie3 = [];
 var avoir = document.querySelectorAll('.sidebar-col.col-md-4.col-sm-4 .c-widget-content.style-1 .widget-content > div');
 for (av of avoir) {
     ...
     var image = av.querySelector('.popular-img.widget-thumbnail.c-image-hover > a > img').dataset.src.replace('-75x106', '');
     image = new ModuleRequest(image, 'get', emptyKeyValue, null);
 ...
 output.push(new Output(CellDesings.wide11, Orientation.horizontal, DefaultLayouts.wideFull, Paging.leading, new Section('Top du jour :', true), null, sortie3));
 output.push(new Output(CellDesings.wide6, Orientation.horizontal, DefaultLayouts.longDoubletsFull, Paging.none, new Section('Exclusivités Origines :', true), null, sortie1));
 output.push(new Output(CellDesings.normal2, Orientation.horizontal, DefaultLayouts.longTripletsDouble, Paging.leading, new Section('Dernière mise a jour :', true), null, sortie2));
