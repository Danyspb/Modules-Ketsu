    ...
    for (m of mem){
     ...
     var info = new Data(image, chap ,title, '', types, '', '', false, link);
     sortie1.push(info);
    }

    var crap = document.querySelectorAll('.serieslist.pop.wpop.wpop-weekly ul li ');
    for(c of crap){
      var image = c.querySelector('.imgseries img').src;
      image = new ModuleRequest(image, 'get', emptyKeyValue, null);
      var link = c.querySelector('.imgseries a').href;
      link = new ModuleRequest(link, 'get', emptyKeyValue, null);
      var title = c.querySelector('.leftseries h2').textContent;
    ...
    output.push(new Output(CellDesings.wide6, Orientation.horizontal, DefaultLayouts.longDoubletsFull, Paging.none,new Section('Top Weekly :', true), null, sortie2));
