...
// SCROLL TO LINE 274 TO START CODING.
...
- What you need to do is to create a Search Object and save it as a json string on the div with the id: ketsu-final-data, to be more precise the one saved on the variable called savedData below this comment.
...
let foun = document.querySelectorAll('.row .media-manga.media ');
for (f of foun){
            let image = f.querySelector('.media-left a img').src;
            image = new ModuleRequest(image, 'get', emptyKeyValue, null);
            let link = f.querySelector('.media-left a').href;
            link = new ModuleRequest(link, 'get', emptyKeyValue, null);
            let title = f.querySelector('.media-heading a').textContent;
            donnes.push(new Data(image, title, '','','','','',false,link));
    }
output.push(new Output(CellDesings.wide11, Orientation.vertical, DefaultLayouts.none, Paging.none, new Section('', false), null, donnes));
